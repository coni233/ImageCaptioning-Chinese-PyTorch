...
def caption_image_beam_search(encoder, decoder, image_path, word_map, beam_size=3):
    ...
    if len(img.shape) == 2:  # 如果是灰度图像，转换成三通道
    ...
    image = transform(img).to(device)  # 转换为tensor并移动到适当的设备
...
def visualize_att(image_path, seq, alphas, rev_word_map, i, smooth=True):
    ...
    plt.savefig('images/out_{}.png'.format(i))
    plt.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', '-m', default='models/best_checkpoint.pth.tar', help='path to model')
